...
const defaultOnConfirm = async (dontShowAgain?: boolean): Promise<void> => {
    return new Promise((resolve) => {
        setTimeout(() => {
            alert(`ลบสำเร็จ! ${dontShowAgain ? '(ไม่แสดงอีก)' : ''}`);
            resolve();
        }, 1000);
    });
};
        ...
        <div className="bg-white rounded-3xl shadow-lg max-w-2xl w-full p-8 animate-in zoom-in-95 duration-300">
            ...
            <div className="flex items-start justify-between mb-8">
                <div className="flex items-start space-x-6">
                    {/* Icon */}
                    <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0 mt-1 transition-all duration-200 hover:scale-105">
                        <Trash2 className="w-8 h-8 text-red-600 transition-all duration-300" />
                    </div>
...
const DeleteContainer: React.FC<DeleteContainerProps> = ({
    message = "ลบข้อมูล",
    subMessage = "คุณต้องการลบข้อมูลนี้ออกจากระบบหรือไม่?\nการกระทำนี้จะไม่สามารถย้อนกลับได้",
    showDontShowAgain = true,
    confirmText = "ลบ",
    cancelText = "ยกเลิก",
